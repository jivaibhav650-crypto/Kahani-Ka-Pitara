...
async function verifyPassword(password, stored) {
  ...
  const h = await passwordHash(password, parts[0]);
  return h === stored;
}
...
async function currentUser(req, env) {
  ...
  const token = (m.match(/(?:^|; )kkp_session=([^;]+)/) || [])[1];
...
async function seedIfEmpty(env) {
  ...
  for (const r of rows) {
    await env.DB.prepare(
      "INSERT OR IGNORE INTO stories(id,title,category,emoji,excerpt,content,lesson) VALUES(?,?,?,?,?,?,?)"
    ).bind(...r).run();
  }
}

async function doLogin(id, env) {
  const token = uid() + uid();
  const th = await sha256(token);
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === "story" && request.method === "GET") {
      ...
      await env.DB.prepare(
        "UPDATE stories SET views=COALESCE(views,0)+1 WHERE id=?"
      ).bind(id).run();
    ...
    if (path === "submit" && request.method === "POST") {
      ...
      await env.DB.prepare(
        ...
        (id,user_id,author_name,title,category,excerpt,content,lesson,image_url,video_url,status)
        VALUES(?,?,?,?,?,?,?,?,?,?,?)`
      ).bind(
        uid(),
        u?.id || null,
    ...
    if (path === "chat" && request.method === "GET") {
      ...
      const r = await env.DB.prepare(
        `SELECT c.id,c.message,c.created_at,u.name,u.email
    ...
    if (path === "analytics" && request.method === "GET") {
      ...
      const today = await env.DB.prepare(
        "SELECT COUNT(*) n FROM page_views WHERE date(created_at)=date('now')"
      ).first();
    ...
    if (
      ...
      if (b.id) {
        ...
        await env.DB.prepare(
          `INSERT INTO stories
           (id,title,category,emoji,excerpt,content,lesson,image_url,video_url)
           VALUES(?,?,?,?,?,?,?,?,?)`
        ).bind(
          id,
          ...row
    ...
    if (
      ...
      if (b.id) {
        ...
        await env.DB.prepare(
          `INSERT INTO posts
           (id,type,title,category,excerpt,content,image_url,status,published_at)
           VALUES(?,?,?,?,?,?,?,?,?)`
        ).bind(
          id,
          ...vals
        ...
        await env.DB.prepare(
          `INSERT OR IGNORE INTO stories
           (id,title,category,emoji,excerpt,content,lesson,
            image_url,video_url,views,created_at)
           VALUES(?,?,?,?,?,?,?,?,?,?,?)`
        ).bind(
          String(s.id),
          s.title || autoTitle(content),
          s.category || "कहानी",
          s.emoji ||
            emojis[s.category] ||
            "📖",
      ...
      for (const p of b.posts || []) {
        await env.DB.prepare(
          `INSERT OR IGNORE INTO posts
           (id,type,title,category,excerpt,content,
